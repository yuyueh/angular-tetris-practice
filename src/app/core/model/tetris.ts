...
export class Tetris extends ImmutableObject {
    ...
    clearCurrentPiece(): Tetris {
        const matrix = this.current.positionOnGridWithoutOutside.reduce(
            (o, [y, x]) => {
    ...
    isCollidedWithSolid(): boolean {
        return this.current.positionOnGridWithoutOutside.some(
            ([y, x]) => this.matrix[y][x].isFilled
        );
    }
    ...
    private _dropLine(
        matrix: DeepReadonlyArray<Tile[]>
    ): DeepReadonlyArray<Tile[]> {
        const aliveRow = (matrix as Tile[][])
            .map((row) => (row.every((col) => col.isSolid) ? null : row))
            .filter(notEmpty);

        return new Array(20).fill(undefined).map((_, i) => {
            return GAME_HEIGHT - i > aliveRow.length
                ? MatrixUtil.getDefaultRow()
                : aliveRow[i - GAME_HEIGHT + aliveRow.length];
